...
namespace AirTrafficMonitoring.Tests.Integration.Step2_SeparationEvents
{
	...
	class Step2_6
	{
		...
		private ISeparationEventListFormatter _formatter;

		[SetUp]
		public void Init()
		{
			_formatter = new SeparationEventListFormatter();
		...
		[Test]
		public void GetFormattedSeparationEvents()
		{
			//Arrange
			var testSeprationEvent = new SeparationEvent(_tag1, _tag2, _dateNow);
			List<SeparationEvent> separationEventsList = new List<SeparationEvent> {testSeprationEvent};

			//Ack
			//Adding Events to list
			_uut.CheckForSeparationEvents(_list);

			//Assert
			Assert.AreEqual(_formatter.Format(separationEventsList) , _uut.GetFormattedSeparationEvents().ToString());
